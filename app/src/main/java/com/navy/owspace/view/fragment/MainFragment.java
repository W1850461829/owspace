package com.navy.owspace.view.fragment;
...
public class MainFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View root = inflater.inflate(R.layout., container, false);
    ...
    public void onResume() {
        ...
        if (model == 5) {
            ...
            Glide.with(getContext()).load(item.getThumbnail()).centerCrop().into(homeAdvertiseIv);
        } else {
            pagerContent.setVisibility(View.VISIBLE);
            homeAdvertiseIv.setVisibility(View.GONE);
            title = item.getTitle();
            Glide.with(getContext()).load(item.getThumbnail()).centerCrop().into(imageIv);
